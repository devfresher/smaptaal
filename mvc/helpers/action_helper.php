...
function btn_view($uri, $name) {
    ...
    if(visibleButton($uri)) {
        ...
        return anchor($uri, "<span class='glyphicon glyphicon-eye-open' aria-hidden='true'></span>", "class='btn btn-success btn-sm  btn-circle  mrg' data-placement='top' data-toggle='tooltip' data-original-title='".$name."'");
...
function btn_edit($uri, $name) {
    ...
    if(visibleButton($uri)) {
        ...
        return anchor($uri, "<span class='glyphicon glyphicon-pencil' aria-hidden='true'></span>", "class='btn btn-warning btn-sm  btn-circle mrg' data-placement='top' data-toggle='tooltip' data-original-title='".$name."'");
    ...
    if(visibleButton($uri)) {
        ...
        return anchor($uri, "<span class='glyphicon glyphicon-remove' aria-hidden='true'></span>",
            ...
            array(
                ...
                'class' => 'btn btn-danger btn-sm  btn-circle mrg',
...
function visibleButton($uri) {
    ...
    if($permission == 'liveclass_view')
    {

        return TRUE;

    }
    elseif(permissionChecker($permission)) {
    ...
    foreach ($nodes as $key => $node) {
        ...
        if( permissionChecker($node['link']) || ($node['link'] == '#' && $f) ) {
            ...
            $menu .= '<li class="'.($f ? $treeView : '').$active.'">';
                ...
                $menu .= anchor($node['link'], '<img src="'.base_url().'/assets/icons/'.($node['icon'] != NULL ? $node['icon'] : 'fa-home').'" width="22"> <span>'. ($CI->lang->line('menu_'.$node['menuName']) != NULL ? $CI->lang->line('menu_'.$node['menuName']) : $node['menuName']).'</span> '.($f ? $leftIcon : ''));
...
function profileviewimage($photoname, $srcpath = NULL) {
    ...
    if($srcpath == NULL) {
        ...
        if($photoname != NULL) {
            ...
            if(file_exists(FCPATH.'uploads/images/'.$img)) {
...
function inputCall($inputName = NULL)
{
    $jsonString = stream_get_contents(fopen('php://input', 'r'));
    $decoded = json_decode($jsonString, true);

    if ($inputName != NULL) {
        return $decoded[$inputName];
    } else {
        return $decoded;
    }
}
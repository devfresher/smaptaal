...
*/
	...
	protected $_versionCheckingUrl = 'http://demo.limpids.net/autoupdate/update/index';
	protected $_updateFileUrl = 'http://demo.limpids.net/autoupdate/updatefiles/school/';
	protected $_successUrl = 'http://demo.limpids.net/autoupdate/update/success';
    ...
    private function postData()
	{
		...
		if(!empty($updates)) {
			$postDatas = array(
				'username' => !empty($this->data['siteinfos']) ? $this->data['siteinfos']->purchase_username : '', 
				'purchasekey' => !empty($this->data['siteinfos']) ? $this->data['siteinfos']->purchase_code : '',
				'domainname' => base_url(),
				'email' => !empty($this->data['siteinfos']) ? $this->data['siteinfos']->email : '',
	...
	private function htmlDesign($versionChecking, $versionShow = TRUE)
	{
		...
		echo '<html>';
			echo '<head>';
				...
				echo '<link href="'.base_url($this->data['backendThemePath'].'/limpids.css').'" rel="stylesheet">';
    ...
    private function databaseUpdate()
    {
    	...
    	if(file_exists($this->_downloadExtractPath.'limpids.json')) {
	    	$limpidsString = file_get_contents($this->_downloadExtractPath.'limpids.json');
			if(!empty($limpidsString)) {
				$limpidsJsonArray = json_decode($limpidsString, true);
				if(isset($limpidsJsonArray['database']['status']) && strtolower($limpidsJsonArray['database']['status']) != 'no') {
					if(isset($limpidsJsonArray['filename']) && !empty($limpidsJsonArray['filename'])) {
						$this->sqlGenerator($limpidsJsonArray['filename']);
					}
				}

				if(isset($limpidsJsonArray['version']) && !empty($limpidsJsonArray['version'])) {
					$returnArray['status'] = true;
					$returnArray['version'] = $limpidsJsonArray['version'];
					$returnArray['message'] = 'Success';
				} 
			} else {
    			$returnArray['message'] = 'limpids.json content is empty'; 
			}
    	} else {
    		$returnArray['message'] = 'limpids.json file not found'; 
    	}
		return (object) $returnArray;
    }
    ...
    {
    	$limpidsString = file_get_contents($this->_downloadExtractPath.'limpids.log');
		if(!empty($limpidsString)) {
			return $limpidsString;
		}
		return '';
    }

		...
		if(permissionChecker('certificatereport')) {
			if((int)$userID && (int)$usertypeID && (int)$templateID && (int)$schoolyearID && (int)$classID) {
				...
				if(!empty($student) && !empty($usertype) && !empty($template) && !empty($schoolyear)) {
					...
					$tagArray['[class]'] 		= !empty($tagClasses) ? $tagClasses->classes : '';
					$tagArray['[section]'] 		= !empty($tagSection) ? $tagSection->section : '';
					$tagArray['[group]'] 		= !empty($tagGroup) ? $tagGroup->group : '';
					$tagArray['[optional_subject]'] = !empty($tagSubject) ? $tagSubject->subject : '';

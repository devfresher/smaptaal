...
*/
    ...
    public function unique_givenamount($postValue) {
    	...
    	if($this->_amountgivenstatus == '') {
	    	...
	    	if($this->session->userdata('usertypeID') == 1 || $this->session->userdata('usertypeID') == 5 || $this->session->userdata('usertypeID') == 3) {
    ...
    public function check_valid_paymentmethod() {
    	$usertypeID = $this->session->userdata('usertypeID');
    	$paymentMethod = ['Cash', 'Cheque', 'Paypal', 'Stripe', 'Payumoney', 'Voguepay','paystack'];
    ...
    public function unique_paymentmethod() {
        if($this->input->post('paymentmethodID') === '0') {
            ...
            if($this->input->post('paymentmethodID') == 'Cash' || $this->input->post('paymentmethodID') == 'Cheque') {
                ...
                return TRUE;
            } elseif ($this->input->post('paymentmethodID') == 'paystack' && $api_config['paystack_status'] == TRUE ){
                return TRUE;
            }else {
    ...
    public function payment() {
        if(permissionChecker('invoice_view')) {
            ...
            if((int) $id) {
            	$maininvoice = $this->maininvoice_m->get_single_maininvoice(array('maininvoiceID' => $id));
//                echo '<pre>' , var_dump($maininvoice) , '</pre>'; exit;
            	if(!empty($maininvoice)) {
		                            ...
		                            if($this->form_validation->run() == FALSE) {
		                            	...
		                            	if($this->input->post('paymentmethodID') == 'Cash' || $this->input->post('paymentmethodID') == 'Cheque') {
								            ...
								            if($this->input->post('paymentmethodID') == 'Paypal') {
                                                ...
                                                $this->voguepay();
                                            } elseif($this->input->post('paymentmethodID') == 'paystack'){
								                $this->paystack($email);
                                            }else {
    ...
    public function  paystack($email)
    {
...
//        }
        ...
        $this->item_data    = $this->post_data;
        $amount = number_format($this->item_data['amount'] + $this->item_data['fine'], 2, '.', '');
        $amount = $amount * 100;
        $this->paystack_lib->add_field('email', $email); // user email (required)
        $this->paystack_lib->add_field('amount', $amount); // amount (required)

        $this->paystack_lib->add_field('callback_url', base_url('invoice/verify')); // callback (required for verifaction)
